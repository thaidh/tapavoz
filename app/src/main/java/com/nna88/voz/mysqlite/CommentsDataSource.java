...
import com.nna88.voz.util.DBAdapter1;
...
public class CommentsDataSource {
    ...
    public void addBookmark(Thread thread) {
        ...
        contentValues.put(MySQLiteHelper.COLUMN_FORUM, BuildConfig.FLAVOR);
        contentValues.put(MySQLiteHelper.COLUMN_LASTPOST, thread.LastPost());
        if (thread.Reply() == null) {
            contentValues.put(MySQLiteHelper.COLUMN_PAGE, BuildConfig.FLAVOR);
        } else {
            contentValues.put(MySQLiteHelper.COLUMN_PAGE, thread.Reply());
        }
        if (thread.View() == null) {
            contentValues.put(MySQLiteHelper.COLUMN_VIEW, BuildConfig.FLAVOR);
        } else {
            contentValues.put(MySQLiteHelper.COLUMN_VIEW, thread.View());
        }
        if (thread.UrlLastPosst() == null) {
            contentValues.put(MySQLiteHelper.COLUMN_URLFIRSTNEWS, BuildConfig.FLAVOR);
        } else {
